...
class LDAVI:
    ...
    def _e_step(self, e_max_iter, meanchangethresh):
        ...
        for doc_idx, doc in enumerate(self._docs): #for each document d 
            ids = doc.wordids
            cts = doc.wordcts 
            ...
            for _ in range(e_max_iter):
                ...
                expElogthetad = numpy.exp(self._dirichlet_expectation(gammad))
                phi = numpy.dot(expElogthetad,expElogbetad)

                gammad = self._alpha + expElogthetad * numpy.dot(cts/phi,expElogbetad.T)
            ...
            #ADD
            sstats[:,ids] += numpy.outer(expElogthetad.T,cts/phi)
        # Compute sufficient statistics for updating lambda (sstats) for M step
