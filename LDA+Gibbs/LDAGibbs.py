    ...
    def _gibbs_sampling(self):
        ...
        for di in range(self.D):
            ...
            for wi in range(len(doc)):
                ...
                #Step 1 : Calculate Document probability
                doc_prob = np.divide(self.DK[di,:]+self.alpha,np.sum(self.DK[di,:])+self.alpha*self.K)
                #Step 2 : Calculate Word probability
                word_prob = np.divide(self.WK[word,:]+self.beta,np.sum(self.WK[:,:],axis = 0)+self.beta*self.W)

                # 2. Sample
                prob_vec = np.multiply(doc_prob,word_prob)
